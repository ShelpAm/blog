...
- 最左推导: 任何一步推导都是对**最左**非终结符的推导。
- 最右推导: 任何一步推导都是对**最右**非终结符的推导。

## Types of grammars

1. Type-0 (recursively enumerable) grammar: $\gamma \to \alpha$ ($\gamma$ not empty)
1. Type-1 (context-sensitive) grammar: $\alpha A \beta \to \alpha \gamma \beta$
1. Type-2 (context-free) grammar: $A \to \alpha$
1. Type-3 (regular) grammar: $A \to a \mid Aa$ (left regular) or $A \to a \mid aB$ (right regular)
